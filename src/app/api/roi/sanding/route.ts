...
function buildHtml(data: {
  ...
  products: Array<{ id: string; name: string; size: string; unitsPerWeek: number; passes: number }>;
...
<title>Sanding machine ROI &#8212; salgsrapport</title>
</head>
<body style="${CSS.body}">
  <div style="${CSS.card}">
    <h1 style="${CSS.h1}">Sanding machine ROI</h1>
...
export async function GET() {
  const html = buildHtml({
    ...
    products: [{ id: "Hinge Door", name: "", size: "702 × 368 × 17 mm", unitsPerWeek: 1750, passes: 2 }],
...
<div style="background:#fff;border-radius:8px;padding:28px 32px;max-width:560px;margin:0 auto;color:#1a1d22;">
  <h2 style="margin:0 0 16px;font-size:18px;color:#0e2238;">Ny ROI-forespørgsel — sanding machine</h2>
  ...
  const bodyText = [
    `Ny ROI-forespørgsel — sanding machine`,
